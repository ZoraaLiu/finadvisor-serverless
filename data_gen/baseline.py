...
def _cap_saving(save: float, spend_m: float) -> float:
    # Suggest between 20 and max(50, 30% of monthlyized spend)
...
def baseline_tips(summary: Dict) -> List[str]:
    ...
    if income_m > 0:
        current_savings_rate = max(0.0, _monthlyize(net_win, window_days) / income_m)
        if current_savings_rate < 0.10:
            target = _cap_saving(round(income_m * 0.10), spend_m)
            tips.append(f"Automate a transfer the day after payday: move ~{_fmt_currency(target, cur)}/month to savings.")

    # 2) Recurring/subscriptions check
    if income_m > 0 and (recurring_m / income_m) > 0.08:
        est = _cap_saving(round(recurring_m * 0.10), spend_m)
        tips.append(f"Recurring charges look high; cancel one unused subscription to save ~{_fmt_currency(est, cur)}/month.")
    elif subs_est >= 3 and recurring_m >= 20:
        est = _cap_saving(round(recurring_m * 0.06), spend_m)
        tips.append(f"You have {subs_est} recurring items; audit and drop one to save ~{_fmt_currency(est, cur)}/month.")
    ...
    if spend_m > 0 and dining_share > 0.20:
        ...
        tips.append(f"Dining is {int(dining_share*100)}% of spend; set a weekly budget to save ~{_fmt_currency(save, cur)}/month.")

    # 4) Shopping clamp (cap ~18%)
    if spend_m > 0 and shopping_share > 0.18:
        excess = (shopping_share - 0.18) * spend_m
        save = _cap_saving(round(excess * 0.5), spend_m)
        tips.append(f"Shopping is {int(shopping_share*100)}% of spend; 48-hour rule + monthly cap can save ~{_fmt_currency(save, cur)}/month.")

    # 5) Delivery frequency (normalize by window length)
    weeks = max(1, math.ceil(window_days / 7))
    if delivery >= (1 * weeks + 1):
        tips.append("Limit food delivery to once per week; batch-cook 2 meals to cut fees and service charges.")
